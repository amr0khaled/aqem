...
void main() async {
  // Ensure Flutter bindings are initialized before accessing assets
...
class _AzkarScreenState extends State<AzkarScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    return Directionality(textDirection: .rtl,child:
    Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text("الأدعية والأذكار",style: TextStyle(color: Colors.white,fontSize: 24,height: 1.3),),
            Text("حصن المسلم اليومي",style: TextStyle(color: Colors.white,fontWeight: .w400,fontSize: 14,height: 1.4,),),
          ],
        ),
        actions: [
          SpecialIcon(content: Text('🤲'),
      ...
      body: Stack(children: [
        AzkarCountStage(
          ...
          , firstWidgetNumber:
          viewIndex == 0 ?categories.length
              :(viewIndex == 1 ?
          (
              category == DuaCategory.sabah ? duaData.where((duaRecord) => duaRecord.category == DuaCategory.sabah).length:
              (
                  category == DuaCategory.masaa ? duaData.where((duaRecord) => duaRecord.category == DuaCategory.masaa).length:
                  (
                    (
                        category == DuaCategory.hefz ? duaData.where((duaRecord) => duaRecord.category == DuaCategory.hefz).length:

                      (
                          category == DuaCategory.nawm ? duaData.where((duaRecord) => duaRecord.category == DuaCategory.nawm).length:

                          (
                              category == DuaCategory.daily ? duaData.where((duaRecord) => duaRecord.category == DuaCategory.daily).length:

                                (category == DuaCategory.salah ? duaData.where((duaRecord) => duaRecord.category == DuaCategory.salah).length:
          ...
          completedNumber: duaData.where((duaRecord)=>duaRecord.isComplete).length,
